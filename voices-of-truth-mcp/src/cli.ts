...
import { getStore, setStore, Scholar, Category, Country } from './schema.js';
...
switch (command) {
  ...
  case 'add-category': {
    const store = getStore();
    const newCat: Category = { id: store.categories.length + 1, en: args[1] || '', ar: args[2] || '' };
  ...
  case 'add-country': {
    const store = getStore();
    const newCountry: Country = { id: store.countries.length + 1, en: args[1] || '', ar: args[2] || '' };
  ...
  case 'search': {
    ...
    const results = store.scholars.filter(s =>
