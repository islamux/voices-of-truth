...
server.registerTool('list_scholars', {
    ...
    if (categoryId !== undefined)
        scholars = scholars.filter(s => s.categoryId === categoryId);
    if (countryId !== undefined)
        scholars = scholars.filter(s => s.countryId === countryId);
    if (language)
        scholars = scholars.filter(s => s.language.includes(language));
...
server.registerTool('get_scholar', {
    ...
    if (!scholar)
        return { content: [{ type: 'text', text: 'Scholar not found' }] };
...
server.registerTool('search_scholars', {
    ...
    let results = store.scholars.filter(s => s.name.en?.toLowerCase().includes(q) ||
        s.name.ar?.includes(q) ||
        s.bio?.en?.toLowerCase().includes(q) ||
        s.bio?.ar?.includes(q));
    if (lang)
        results = results.filter(s => s.language.includes(lang));
...
server.registerTool('get_statistics', {
    ...
    const stats = {
        ...
        by_category: Object.fromEntries(store.categories.map(c => [c.en, store.scholars.filter(s => s.categoryId === c.id).length])),
        by_language: Object.fromEntries([...new Set(store.scholars.flatMap(s => s.language))].map(l => [l, store.scholars.filter(s => s.language.includes(l)).length])),
        by_country: Object.fromEntries(store.countries.map(c => [c.en, store.scholars.filter(s => s.countryId === c.id).length]))
    };
...
server.registerTool('update_scholar', {
    ...
    if (idx === -1)
        return { content: [{ type: 'text', text: 'Scholar not found' }] };
    store.scholars[idx] = { ...store.scholars[idx], ...updates };
    saveDatabase();
...
server.registerTool('add_category', {
    ...
    loadDatabase();
    const newCat = { id: store.categories.length + 1, en, ar };
...
server.registerTool('add_country', {
    ...
    loadDatabase();
    const newCountry = { id: store.countries.length + 1, en, ar };
...
server.registerTool('localize_scholar', {
    ...
    if (!scholar)
        return { content: [{ type: 'text', text: 'Scholar not found' }] };
    if (name)
        scholar.name = { ...scholar.name, ...name };
    saveDatabase();
    return { content: [{ type: 'text', text: JSON.stringify(scholar) }] };
});
