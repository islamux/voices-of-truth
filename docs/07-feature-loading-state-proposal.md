...
> **Note:** This document describes a proposed feature adapted for the current architecture. The `useScholars` hook no longer exists; filtering logic lives in the server component (`src/app/[locale]/page.tsx`) and interactivity uses `FilterContext`.
